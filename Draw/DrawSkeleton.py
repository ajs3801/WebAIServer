...
def DrawSkeleton(image,landmark_pose,COLOR):
  ...
  if (RIGHT_SHOULDER_X!=0 and RIGHT_ELBOW_X!=0):
    # 오른쪽 어깨 - 오른쪽 팔꿈치
    cv2.line(image, (RIGHT_SHOULDER_X, RIGHT_SHOULDER_Y) , (RIGHT_ELBOW_X,RIGHT_ELBOW_Y), COLOR, THICKNESS, cv2.LINE_AA)
  
  if (RIGHT_WRIST_X!=0 and RIGHT_ELBOW_X!=0):
    # 오른쪽 손목 - 오른쪽 팔꿈치
    cv2.line(image, (RIGHT_WRIST_X , RIGHT_WRIST_Y) , (RIGHT_ELBOW_X, RIGHT_ELBOW_Y), COLOR, THICKNESS, cv2.LINE_AA)
  
  if (RIGHT_SHOULDER_X!=0 and CENTER_SHOULDER_X!=0):
    # 오른쪽 어깨 - 중어깨
    cv2.line(image, (RIGHT_SHOULDER_X , RIGHT_SHOULDER_Y) , (CENTER_SHOULDER_X, CENTER_SHOULDER_Y), COLOR, THICKNESS, cv2.LINE_AA)

  if (CENTER_SHOULDER_X!=0 and LEFT_SHOULDER_X!=0):
    # 중어깨 - 왼쪽 어깨
    cv2.line(image, (CENTER_SHOULDER_X , CENTER_SHOULDER_Y) , (LEFT_SHOULDER_X, LEFT_SHOULDER_Y), COLOR, THICKNESS, cv2.LINE_AA)
  
  if (LEFT_SHOULDER_X!=0 and LEFT_ELBOW_X!=0):
    # 왼쪽 어깨 - 왼쪽 팔꿈치
    cv2.line(image, (LEFT_SHOULDER_X, LEFT_SHOULDER_Y) , (LEFT_ELBOW_X, LEFT_ELBOW_Y), COLOR, THICKNESS, cv2.LINE_AA)
  
  if (LEFT_ELBOW_X!=0 and LEFT_WRIST_X!=0):
    # 왼쪽 팔꿈치 - 왼쪽 손목
    cv2.line(image, (LEFT_ELBOW_X, LEFT_ELBOW_Y) , (LEFT_WRIST_X, LEFT_WRIST_Y), COLOR, THICKNESS, cv2.LINE_AA)
  if (CENTER_SHOULDER_X!=0 and CENTER_HIP_X!=0):  
    # 중어꺠 - 중덩이
    cv2.line(image, (CENTER_SHOULDER_X , CENTER_SHOULDER_Y) , (CENTER_HIP_X, CENTER_HIP_Y), COLOR, THICKNESS, cv2.LINE_AA)
  if (RIGHT_HIP_X!=0 and CENTER_HIP_X!=0):
    # 오른쪽 엉덩이 - 중덩이
    cv2.line(image, (RIGHT_HIP_X , RIGHT_HIP_Y) , (CENTER_HIP_X, CENTER_HIP_Y), COLOR, THICKNESS, cv2.LINE_AA)
  if (LEFT_HIP_X!=0 and CENTER_HIP_X!=0):
    # 왼쪽 엉덩이 - 중덩이
    cv2.line(image, (LEFT_HIP_X , LEFT_HIP_Y) , (CENTER_HIP_X, CENTER_HIP_Y), COLOR, THICKNESS, cv2.LINE_AA)
  if (RIGHT_HIP_X!=0 and RIGHT_KNEE_X!=0):
    # 오른쪽 엉덩이 - 오른쪽 무릎
    cv2.line(image, (RIGHT_HIP_X , RIGHT_HIP_Y) , (RIGHT_KNEE_X, RIGHT_KNEE_Y), COLOR, THICKNESS, cv2.LINE_AA)
  if (RIGHT_KNEE_X!=0 and RIGHT_ANKLE_X!=0):
    # 오른쪽 무릎 - 오른쪽 발목
    cv2.line(image, (RIGHT_KNEE_X , RIGHT_KNEE_Y) , (RIGHT_ANKLE_X, RIGHT_ANKLE_Y), COLOR, THICKNESS, cv2.LINE_AA)
  if (LEFT_HIP_X!=0 and LEFT_KNEE_X!=0):
    # 왼쪽 엉덩이 - 왼쪽 무릎
    cv2.line(image, (LEFT_HIP_X, LEFT_HIP_Y) , (LEFT_KNEE_X, LEFT_KNEE_Y), COLOR, THICKNESS, cv2.LINE_AA)
  if (LEFT_KNEE_X!=0 and LEFT_ANKLE_X!=0):
    # 왼쪽 무릎 - 왼쪽 발목
    cv2.line(image, (LEFT_KNEE_X , LEFT_KNEE_Y) , (LEFT_ANKLE_X, LEFT_ANKLE_Y), COLOR, THICKNESS, cv2.LINE_AA)
